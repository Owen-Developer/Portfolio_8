...
for(let i = 0; i <= 500; i++){
    ...
    newSquare.addEventListener("mouseenter", () => {
        newSquare.style.backgroundColor = "hsl(0, 0%, 83%)";
    });
...
document.querySelector(".btn-home").addEventListener("mouseenter", () => {
    document.querySelector(".btn-home").style.border = "1px solid hsl(0, 0%, 10%)";
...
document.querySelector(".btn-home").addEventListener("mouseleave", () => {
    ...
    document.querySelector(".btn-home").style.backgroundColor = "hsl(0, 0%, 10%)";
    document.querySelector(".btn-home-txt").style.color = "hsl(0, 0%, 90%)";
    document.querySelector(".home-arr").style.color = "hsl(0, 0%, 90%)";
});
...
document.querySelectorAll(".feat-box").forEach(box => {
    box.addEventListener("mouseenter", () => {if(window.innerWidth > 1130){
        ...
        box.querySelector(".feat-shadow").style.background = "linear-gradient(to left, hsla(0, 0%, 90%, 0.93), hsla(0, 0%, 76%, 0.93))";
    ...
    box.addEventListener("mouseleave", () => {if(window.innerWidth > 1130){
        ...
        box.querySelector(".feat-shadow").style.background = "linear-gradient(to left, hsla(0, 0%, 76%, 0.93), hsla(0, 0%, 90%, 0.93))";
...
document.querySelectorAll(".ques-box").forEach((box, idx) => {
    box.addEventListener("click", () => {
        ...
        if(!quesBoxes[idx]){
            ...
            if(window.innerWidth < 1130){
                box.style.background = "hsla(0, 0%, 70%, 0.93)";
            } else {
                box.style.background = "linear-gradient(to right, hsla(0, 0%, 85%, 0.93), hsla(0, 0%, 60%, 0.93))";
            }
            quesBoxes[idx] = true;
            quesSeen[idx] = true;
        } else {
    ...
    box.addEventListener("mouseleave", () => {if(window.innerWidth > 1130){
        if(quesSeen[idx]){
            ...
            box.style.background = "hsla(0, 0%, 80%, 0.93)";
        }
    }});
});
